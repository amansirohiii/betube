...
const VideoCard = ({info}) => {
  ...
  return (
    <div className="p-2 m-2 w-72">
        ...
        {!searchVideos?
          ( <li className='text-gray-500 text-[13px]'>{info?.statistics?.viewCount ? formatCompactNumber(info?.statistics?.viewCount) : 0} views  {(Math.abs(new Date(publishedAt) - new Date()) / (60 * 60 * 24 * 1000)).toFixed(0)} days ago</li>):null}
        </div>
